...
function updateVisibleItems() {
  ...
  for (let row = startRow; row <= endRow; row++) {
    for (let col = startCol; col <= endCol; col++) {
      ...
      img.src = `./img/${itemNum}.png`;
      img.alt = items[itemNum]; // Note: This has an off-by-one index, but matches the video
...
function expandItem(item) {
  ...
  const imgMatch = imgSrc.match(/img\/(\d+)\.png$/);    
  const imgNum = parseInt(imgMatch[1] || 1);
...
const isMobile = viewportWidth < 768; // Check if we're on a small screen
const targetWidth = isMobile ? (viewportWidth * 0.8) : (viewportWidth * 0.4); // 80% on mobile, 40% on desktop
const targetHeight = targetWidth * 1.2; // Keep aspect ratio
...
function closeExpandedItem() {
  if (!isExpanded || !originalPosition) return;
